...
def send_support_payload_to_client(message):
    ...
    resp, return_code = make_logistics_support_payload_request_to_client(support_payload)
...
def send_support_response_to_bap(message):
    log(f"logistics on_support payload: {message}")
    on_support_message_id = message['message_ids']['on_support']
    on_support_payload = get_first_ondc_request(OndcDomain.LOGISTICS, OndcAction('on_support'), on_support_message_id)
