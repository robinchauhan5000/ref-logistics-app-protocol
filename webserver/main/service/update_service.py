...
def make_logistics_update_payload_request_to_client(update_payload):
...
def send_update_payload_to_client(message):
    ...
    resp, return_code = make_logistics_update_payload_request_to_client(update_payload)
...
def send_update_response_to_bap(message):
    log(f"logistics on_update payload: {message}")
    on_update_message_id = message['message_ids']['on_update']
    on_update_payload = get_first_ondc_request(OndcDomain.LOGISTICS, OndcAction('on_update'), on_update_message_id)
