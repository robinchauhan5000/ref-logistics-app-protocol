...
def make_logistics_issue_status_payload_request_to_client(issue_status_payload):
...
def send_issue_status_payload_to_client(message):
    ...
    resp, return_code = make_logistics_issue_status_payload_request_to_client(
...
def send_issue_status_response_to_bap(message):
    log(f"logistics on_issue_status payload: {message}")
    on_issue_status_message_id = message['message_ids']['on_issue_status']
    on_issue_status_payload = get_first_ondc_request(
        OndcDomain.LOGISTICS, OndcAction('on_issue_status'), on_issue_status_message_id)
