...
def send_confirm_payload_to_client(message):
    ...
    resp, return_code = make_logistics_confirm_payload_request_to_client(confirm_payload)
...
def send_confirm_response_to_bap(message):
    log(f"logistics on_confirm payload: {message}")
    on_confirm_message_id = message['message_ids']['on_confirm']
    on_confirm_payload = get_first_ondc_request(OndcDomain.LOGISTICS, OndcAction('on_confirm'), on_confirm_message_id)
