...
def make_logistics_search_payload_request_to_client(select_payload):
    return get_responses_from_client("logistics/search-payload-for-logistics-select", select_payload)


def make_logistics_select_payload_request_to_client(select_payload):
    return get_responses_from_client("client/select", select_payload)


@check_for_exception
def send_select_payload_to_client(message):
    log(f"logistics select payload: {message}")
    select_message_id = message['message_ids']['select']
    select_payload = get_first_ondc_request(OndcDomain.LOGISTICS, OndcAction('select'), select_message_id)
    resp, return_code = make_logistics_select_payload_request_to_client(select_payload)
...
def send_select_response_to_bap(message):
    log(f"logistics on_select payload: {message}")
    on_select_message_id = message['message_ids']['on_select']
    on_select_payload = get_first_ondc_request(OndcDomain.LOGISTICS, OndcAction('on_select'), on_select_message_id)
