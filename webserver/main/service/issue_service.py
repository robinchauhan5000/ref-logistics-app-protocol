...
def make_logistics_issue_payload_request_to_client(issue_payload):
...
def send_issue_payload_to_client(message):
    ...
    resp, return_code = make_logistics_issue_payload_request_to_client(
        issue_payload)
...
def send_issue_response_to_bap(message):
    log(f"logistics on_issue payload: {message}")
    on_issue_message_id = message['message_ids']['on_issue']
    on_issue_payload = get_first_ondc_request(
        OndcDomain.LOGISTICS, OndcAction('on_issue'), on_issue_message_id)
