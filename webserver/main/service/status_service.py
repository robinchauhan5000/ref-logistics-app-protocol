...
def send_status_payload_to_client(message):
    ...
    resp, return_code = make_logistics_status_payload_request_to_client(status_payload)
...
def send_status_response_to_bap(message):
    log(f"logistics on_status payload: {message}")
    on_status_message_id = message['message_ids']['on_status']
    on_status_payload = get_first_ondc_request(OndcDomain.LOGISTICS, OndcAction('on_status'), on_status_message_id)
