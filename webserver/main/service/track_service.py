...
def send_track_payload_to_client(message):
    ...
    resp, return_code = make_logistics_track_payload_request_to_client(track_payload)
...
def send_track_response_to_bap(message):
    log(f"logistics on_track payload: {message}")
    on_track_message_id = message['message_ids']['on_track']
    on_track_payload = get_first_ondc_request(OndcDomain.LOGISTICS, OndcAction('on_track'), on_track_message_id)
