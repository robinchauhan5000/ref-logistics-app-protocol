...
def send_init_payload_to_client(message):
    ...
    resp, return_code = make_logistics_init_payload_request_to_client(init_payload)
...
def send_init_response_to_bap(message):
    log(f"logistics on_init payload: {message}")
    on_init_message_id = message['message_ids']['on_init']
    on_init_payload = get_first_ondc_request(OndcDomain.LOGISTICS, OndcAction('on_init'), on_init_message_id)
