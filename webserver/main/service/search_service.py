...
def make_logistics_search_payload_request_to_client(search_payload):
...
def send_search_payload_to_client(message):
    ...
    resp, return_code = make_logistics_search_payload_request_to_client(search_payload)
    log(f"Got response {resp} from client with status-code {return_code}")


@check_for_exception
def send_search_response_to_gateway(message):
    log(f"logistics on_search payload: {message}")
    on_search_message_id = message['message_ids']['on_search']
    on_search_payload = get_first_ondc_request(OndcDomain.LOGISTICS, OndcAction('on_search'), on_search_message_id)
