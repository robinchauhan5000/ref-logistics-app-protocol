...
def send_cancel_payload_to_client(message):
    ...
    resp, return_code = make_logistics_cancel_payload_request_to_client(cancel_payload)
...
def send_cancel_response_to_bap(message):
    log(f"logistics on_cancel payload: {message}")
    on_cancel_message_id = message['message_ids']['on_cancel']
    on_cancel_payload = get_first_ondc_request(OndcDomain.LOGISTICS, OndcAction('on_cancel'), on_cancel_message_id)
